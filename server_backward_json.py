...
"""
    ...
    async def search_file(self, scale_path: str, session_id: str = None, limit: int = 30, extract_from_top: int = 8,
        ...
        """Search for relevant pages using ColPali (backward translation, no familiarity check)"""
        try:
            ...
            if not search_result:
                logger.warning("Qdrant search returned no results — collection may be empty or query vector mismatch")
...
def main(use_memory: bool, port: int, host: str) -> int:
    ...
    async def upload_endpoint(request: Request):
        ...
        # Save to a permanent directory so images can be reloaded after server restart
        pdf_dir = "/project/def-fji/rag_server_backward/pdfs"
        os.makedirs(pdf_dir, exist_ok=True)
        pdf_path = os.path.join(pdf_dir, up.filename)
        try:
            data = await up.read()
            ...
            result_msg = await rag_server.index_pdf(pdf_path)
    ...
    async def translate_endpoint(request: Request):
        """Backward translation with RAG. No familiarity check required."""
        try:
            payload = await request.json()
        except Exception:
            return JSONResponse({"success": False, "error": "Invalid JSON body."}, status_code=400)

        session_id = (payload.get("session_id") or "").strip() or None  # Optional, auto-generated if missing
    ...
    starlette_app = Starlette(
        ...
        routes=[
            ...
            Route("/translate", endpoint=translate_endpoint, methods=["POST"]),  # RAG translation

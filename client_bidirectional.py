...
FORWARD_GUIDELINES = ['who.pdf', 'itc.pdf', 'lee.pdf', 'boateng.pdf', 'johnson.pdf', 'forward.pdf']
BACKWARD_GUIDELINES = ['who.pdf', 'itc.pdf', 'backward.pdf']
...
class RAGClient:
    ...
    def upload_guideline(self, pdf_path: str):
        """Upload a single guideline PDF to the server."""
        with open(pdf_path, "rb") as f:
            files = {"file": (Path(pdf_path).name, f, "application/pdf")}
            response = requests.post(f"{self.base_url}/upload", files=files)
            return response.json()

    def batch_upload_guidelines(self, pdf_paths: list):
        """Upload multiple guideline PDFs."""
        results = []
        for pdf_path in pdf_paths:
            print(f"  Uploading {Path(pdf_path).name}...", end=" ")
            result = self.upload_guideline(pdf_path)
            results.append({
                "filename": Path(pdf_path).name,
                "success": result.get("success", False),
                "message": result.get("message", result.get("error", ""))
            })
            print("✓" if result.get("success") else f"✗ {result.get('error')}")
    ...
    def check_familiarity(self, scale_path: str, model: str):
        """Stage 1: Call /check to get the LLM's familiarity with the scale."""
        payload = {
    ...
    def translate(self, scale_path: str, model: str, temperature: float = 0.7,
        ...
        """Stage 2: Call /translate to perform RAG-based translation."""
    ...
    def back_translate(self, scale_path: str, model: str, temperature: float = 0.7,
                       limit: int = 30, extract_from_top: int = 5):
        """Call the back-translation server's /translate endpoint.
        No familiarity check needed for backward translation."""
        payload = {
            "scale_path": scale_path,
            "model": model,
            "temperature": temperature,
...
def upload_guidelines(client: RAGClient, pdf_paths: list):
    print(f"Uploading {len(pdf_paths)} guideline files...")
    result = client.batch_upload_guidelines(pdf_paths)
    print(f"\nUploaded {result['successful']}/{result['total']} files")
...
def run_param_grid(scale_path: str, models: list = None, temperatures: list = None,
                   ...
                   fwd_guidelines: list = None, back_guidelines: list = None,
    ...
    fwd_guidelines = fwd_guidelines or FORWARD_GUIDELINES
    back_guidelines = back_guidelines or BACKWARD_GUIDELINES
    ...
    # Upload guidelines to each server (separate docs to avoid retrieval contamination)
    print("=" * 60)
    print("Uploading guidelines to FORWARD server")
    print("=" * 60)
    upload_guidelines(fwd_client, fwd_guidelines)

    print("=" * 60)
    print("Uploading guidelines to BACKWARD server")
    print("=" * 60)
    upload_guidelines(back_client, back_guidelines)
...
def interactive_translate():
    ...
    # Upload guidelines
    upload_guidelines(fwd_client, FORWARD_GUIDELINES)
    upload_guidelines(back_client, BACKWARD_GUIDELINES)
